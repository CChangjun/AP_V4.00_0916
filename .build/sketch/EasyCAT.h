#line 1 "C:\\개인폴더\\FirmWare\\무선화FW\\TEST\\revisionBoard\\AP_ver3.31_PEERLED추가TEST\\ApToEthercat_V3.31_20251014\\EasyCAT.h"
...
#define ALIAS_A_Reg             0x0010
#define ALIAS_B_Reg             0x0011
#define ALIAS_M_Reg             0x0012
#define ALIAS_L_Reg             0x0013

#define CONFIG_EEPROM           0x0500
#define CONFIG_EEPROM_CONTROL   0x01
...
#else  
  #define SpiSpeed        16000000    // Modify, org Value 8000000
#endif
...
bool EasyCAT::Init()
{
  ...
  SPI.begin(SCK, MISO, MOSI, SS);   // modify, assign pin namel org SPI.begin();
  ...
  delay(100);                                             // 리셋에 대충 요정도 걸림, 딜레이주면 아래 do-while에서 카운트 한자리수로 끝남
  ...
  Serial.printf("RESET_CTL;   i:%4d, TempLong.Long : %ld, TempLong.Byte[0/3] : %d, OK\n", i, TempLong.Long, TempLong.Byte[0]);
                                                          //                                                       
  ...
  Serial.printf("BYTE_TEST;   i:%4d, TempLong.Long : 0x%08x, OK\n", i, TempLong.Long);
                                                          //                                                            
  if (i == Tout)                                          // time out expired      
  {                                                       //
    SPI.endTransaction();                                 //                        
  ...
  Serial.printf("HW_CFG;      i:%4d, TempLong.Long : 0x%08x, OK\n\n", i, TempLong.Long);
                                                          //
  if (i == Tout)                                          // time out expired      
  {                                                       //
    SPI.endTransaction();                                 //                        
...
void EasyCAT::SPIWriteRegisterDirect (unsigned short Address, unsigned long DataOut)
{
  ...
  UWORD Addr;
  ULONG Data; 
  Addr.Word = Address;    // unsigned short
  Data.Long = DataOut;    // unsigned long

// Serial.printf("\nSPIWrite Address : 0x%04x, DataOut : %ld\n", Address, DataOut);
...
//  Serial.printf("Addr.Byte[1] : %d, Byte[0] : %d, Data.Byte[0] : %d, [1] : %d, [2] : %d, [3] : %d\n", Addr.Byte[1], Addr.Byte[0], Data.Byte[0], Data.Byte[1], Data.Byte[2], Data.Byte[3]);
